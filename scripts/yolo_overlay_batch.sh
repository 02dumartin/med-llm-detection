...
usage() {
    cat <<EOF
Usage:
  bash scripts/yolo_overlay_batch.sh [options]

Options:
  --device <gpu>           GPU id for overlay inference (default: ${DEVICE})
  --conf <float>           prediction confidence threshold (default: ${CONF})
  --iou <float>            prediction IoU threshold (default: ${IOU})
  --save-gt-overlay        also save GT overlays when supported
  --use-saved-pred-txt     draw overlays from existing results/<...>/evaluation/labels
  --force                  rerun jobs even if overlays already exist
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --use-saved-pred-txt) USE_SAVED_PRED_TXT=true; shift ;;
...
run_job() {
    ...
    if ! $FORCE && is_job_complete "$results_dir"; then
        echo "[SKIP] overlay already done: $model_label -> $test_data"
    ...
    mkdir -p "$results_dir/overlay/pred"
    rm -f "$results_dir/overlay/pred/.done"
    $SAVE_GT_OVERLAY && extra_args+=(--save-gt-overlay)
    if $USE_SAVED_PRED_TXT; then
        extra_args+=(--source txt --use-saved-pred-txt)
    else
        extra_args+=(--source live)
    fi

    echo ""
    echo "============================================================"
    echo "[RUN] overlay $model_label -> $test_data"
    ...
    if "$VENV_PY" "$OVERLAY_PY" \
        ...
        "${extra_args[@]}" \
        > "$log_file" 2>&1; then
        touch "$results_dir/overlay/pred/.done"
        echo "[DONE] overlay $model_label -> $test_data"
        append_status "DONE" "$model_label" "$test_data" "$weights" "$results_dir" "$log_file"
    else
        echo "[FAIL] overlay $model_label -> $test_data"
        echo "       log: $log_file"
...
echo "[INFO] Batch overlay start"
echo "  device      : $DEVICE"
echo "  conf / iou  : $CONF / $IOU"
echo "  save_gt     : $SAVE_GT_OVERLAY"
echo "  use_saved   : $USE_SAVED_PRED_TXT"
echo "  force       : $FORCE"
...
echo ""
echo "[DONE] Batch overlay finished."
